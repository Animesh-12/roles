...
namespace DeviceManagement
{
    public class BlogPostQuery
    {
        ...
        public async Task<List<Role>> getUserInformation()
        {
            ...
            cmd.CommandText = @"select  * from db_final.role;";
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"select * from db_final.user_to_role;";
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"SELECT * from db_final.role where db_final.role.role_id = @id";
        ...
        { 
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"SELECT * from db_final.permission where db_final.permission.permission_id = @id";
            cmd.Parameters.Add(new MySqlParameter
            {
                ParameterName = "@id",
                DbType = DbType.Int32,
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"delete from db_final.user_to_role where db_final.user_to_role.user_id = @id and db_final.user_to_role.role_id = @id1; ";
            BindId10(cmd, id, id1);
            return await ReadAllUserRole(await cmd.ExecuteReaderAsync());
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"delete from db_final.role_to_permission  where db_final.role_to_permission.role_id=@id;
                delete from db_final.role where db_final.role.role_id=@id;";
            BindId(cmd, id);
            return await ReadAllEmployee(await cmd.ExecuteReaderAsync());
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"select r.role_id,r.role_name  from db_final.role  r
                        join db_final.user_to_role rp  on
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"select * from  db_final.role where role_id NOT IN (
select r.role_id  from db_final.role  r
                        join db_final.user_to_role rp   on
                    rp.role_id=r.role_id  where rp.user_id=@id); ";
            BindId7(cmd, Id);
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"delete from db_final.role_to_permission  where db_final.role_to_permission.permission_id=@id;
        delete   from db_final.permission where db_final.permission.permission_id=@id;
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"SELECT * FROM db_final.permission;";
            return await ReadAllPermission(await cmd.ExecuteReaderAsync());
        }
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"delete from db_final.role_to_permission where role_id=@id and permission_id=@id1;;";
            BindId4(cmd, id,id1);
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"select r.role_id,r.role_name,p.permission_id,p.permission_name  from db_final.role
            r join db_final.permission p 
            join db_final.role_to_permission rp on r.role_id=rp.role_id and p.permission_id=rp.permission_id  order by rp.role_id ASC";
            return await ReadAllRolePermission(await cmd.ExecuteReaderAsync());
        }
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"select rp.permission_id,p.permission_name  from db_final.permission  p
                        join db_final.role_to_permission rp  on 
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"select * from db_final.permission pp where pp.permission_id NOT IN( 
        select rp.permission_id from db_final.permission p join db_final.role_to_permission rp 
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"insert into db_final.role (role_name) values(@roleName);";
...
// insert the permission
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"insert into db_final.permission (permission_name) values(@permissionName);";
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"insert into db_final.role_to_permission(role_id,permission_id) values(@role,@permission);";
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"update db_final.role SET db_final.role.role_name=@RoleName where role_id = @id;";
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"select u.user_id,u.first_name,u.last_name from db_final.user as u;";
